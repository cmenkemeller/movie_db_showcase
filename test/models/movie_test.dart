...
void main() {
  group('Movie', () {
    ...
    test('parse movie json', () {
      final movie = Movie.fromJson(mockMovieJson);
      expect(movie.id, mockMovieJson['id']);
      expect(movie.title, mockMovieJson['title']);
      expect(movie.releaseDate, mockMovieJson['release_date']);
      expect(movie.voteAverage, mockMovieJson['vote_average']);
      expect(movie.posterPath, mockMovieJson['poster_path']);
      expect(movie.voteCount, mockMovieJson['vote_count']);
    });
  });
}
